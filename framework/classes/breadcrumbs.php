...
if ( ! class_exists( 'Breadcrumb_Trail' ) ) {
    class Breadcrumb_Trail
    {
        ...
        {
            $defaults = array (
                'browse' => esc_html__( 'Browse:' , 'ictu' ) ,
                'aria_label' => esc_attr_x( 'Breadcrumbs' , 'breadcrumbs aria label' , 'ictu' ) ,
                'home' => esc_html__( 'Home' , 'ictu' ) ,
                'error_404' => esc_html__( '404 Not Found' , 'ictu' ) ,
                'archives' => esc_html__( 'Archives' , 'ictu' ) ,
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search' => esc_html__( 'Search results for &#8220;%s&#8221;' , 'ictu' ) ,
                // Translators: %s is the page number.
                'paged' => esc_html__( 'Page %s' , 'ictu' ) ,
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute' => esc_html__( 'Minute %s' , 'ictu' ) ,
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week' => esc_html__( 'Week %s' , 'ictu' ) ,
        ...
        protected function add_items ()
        {
            ...
            if ( is_front_page() ) {
                ...
                elseif ( is_singular() ) {
                    ...
                    if ( $this->args['only_category'] ) {
                        $categories = get_the_terms( $post , 'category' );
                        echo "<pre>";
                        print_r( $categories );
                        echo "</pre>";
                        if ( $categories ) {
                            $this->add_term_archive_items( $categories[0] );
                        }
                    } else {
         ...
         * @access protected
         */
        protected function add_term_archive_items ( $term = null , $term_title = '' )
        {
            global $wp_rewrite;
        ...
        protected function add_minute_hour_archive_items ()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf( $this->labels['archive_minute_hour'] ,
                    get_the_time( esc_html_x( 'g:i a' , 'minute and hour archives time format' , 'ictu' ) ) );
            }
        }
        ...
        protected function add_minute_archive_items ()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf( $this->labels['archive_minute'] ,
                    get_the_time( esc_html_x( 'i' , 'minute archives time format' , 'ictu' ) ) );
            }
        }
        ...
        protected function add_hour_archive_items ()
        {
            ...
            if ( true === $this->args['show_title'] ) {
                $this->items[] = sprintf( $this->labels['archive_hour'] ,
                    get_the_time( esc_html_x( 'g a' , 'hour archives time format' , 'ictu' ) ) );
            }
        }
        ...
        protected function add_day_archive_items ()
        {
            ...
            $year = sprintf( $this->labels['archive_year'] ,
                get_the_time( esc_html_x( 'Y' , 'yearly archives date format' , 'ictu' ) ) );
            $month = sprintf( $this->labels['archive_month'] ,
                get_the_time( esc_html_x( 'F' , 'monthly archives date format' , 'ictu' ) ) );
            $day = sprintf( $this->labels['archive_day'] ,
                get_the_time( esc_html_x( 'j' , 'daily archives date format' , 'ictu' ) ) );
        ...
        protected function add_week_archive_items ()
        {
            ...
            $week = sprintf( $this->labels['archive_week'] ,
                get_the_time( esc_html_x( 'W' , 'weekly archives date format' , 'ictu' ) ) );
            // Add the year item.
            $this->items[] = sprintf( '<a rel="v:url" property="v:title" href="%s">%s</a>' ,
                esc_url( get_year_link( get_the_time( 'Y' ) ) ) , $year );
            // Add the week item.
